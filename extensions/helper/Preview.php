...
*
*/
namespace minerva_gallery\extensions\helper;

use lithium\analysis\Logger;
use \SplFileInfo;
defined('DS') ? null : define('DS', DIRECTORY_SEPARATOR); 	
		
class Preview extends \lithium\template\Helper {
	...
	public function image($source=null, $options = array('size'=>array(75, 75), 'quality'=>85, 'crop'=>false, 'letterbox'=>null, 'force_letterbox_color'=>false, 'sharpen'=>true), $htmlOptions = array()) {	
		...
		// If it starts with /minerva_gallery then the file was stored on disk.
		$source_info = false;
		if(substr($source, 0, 16) == '/minerva_gallery') {
			$this->source = LITHIUM_APP_PATH . DS . 'libraries' . DS . 'minerva_gallery' . DS . 'webroot' . substr($source, 16);
		...
		// TODO: The file may be on Amazon S3 or somewhere else... Or stored in grid.fs
		// Make provisions for each of those cases.
		// TODO: this should probably all be done in maybe the Item model...
		// The Html::image() helper works off URLs so ... Might as well always store a full URL
		// in the database for the item. So during the save process maybe set this...
		// It'll make this helper, and everything else, consistent.
		...
		// THIRD: Set the destination (and create the path)
		// Since all items have a unique _id... and all images have a URL (also unique) this is easy.
		// Nothing will conflict, so we can store all thumbnail cache in one place.
		// We'll md5() the $source for the file name.
		$destination_base_path = LITHIUM_APP_PATH . DS . 'libraries' . DS . 'minerva_gallery' . DS . 'webroot' . DS . 'img' . DS . 'gallery_items' . DS . 'previews' . DS . $thumb_size_x . 'x' . $thumb_size_y . DS;
		// ...and for the helper
		$web_base_path = DS . 'minerva_gallery' . DS .'img' . DS . 'gallery_items' . DS . 'previews' . DS . $thumb_size_x . 'x' . $thumb_size_y . DS;
		...
		$this->_createPath($destination_base_path);
		
		// Set the destination, it's a full path on disk.
		// TODO: Think about in the future saving thumbnails elsewhere like S3, etc.
		$this->destination = $destination_base_path . $source_info->getFilename();
		
		// Set the final web path for the file too (the helper needs it)
		$final_web_path = $web_base_path . $source_info->getFilename();
	    ...
	    // Make sure it's an image that we can use (bmp support isn't added, but could be)
		...
		// TODO: This obviously won't work if the images are stored elsewhere...
		// So we need to find a different cache strategy...Maybe even store a last updated
		// time on the item document...or APC... Maybe have a manual refresh in the admin area.
		// Could check headers if the file is remote for expires info...But that could be a lot of connections.
		...
		if(file_exists($this->destination)) {
			...
			if($existingFile->getMTime() > $source_info->getMTime()) {
				...
				return $this->_context->helper('Html')->image($final_web_path, $htmlOptions);
			}
		}
