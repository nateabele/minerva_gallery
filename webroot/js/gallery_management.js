...
function addGalleryItem(gallery_id, item_id) {
	...
	$.get('/minerva_gallery/items/meta/' + item_id + '.json', function(data) {
...
function removeItemFromGallery(item_id, gallery_id) {
	...
	$.get('/minerva_gallery/items/association/remove/' + item_id + '/' + gallery_id + '.json', function(data) {
...
function editGalleryItem(item_id) {
	...
	$.get('/minerva_gallery/items/meta/' + item_id + '.json', function(data) {
		...
		$('#edit_gallery_item_thumbnail').html('<img src="' + $('#image_for_' + item_id).attr('src') + '" alt="' + $('#image_for_' + item_id).attr('alt') + '" />');
	});
...
function addUnassociatedItem(item_id, gallery_id) {
	...
	$.get('/minerva_gallery/items/association/add/' + item_id + '/' + gallery_id + '.json', function(data) {
		if(data.success == true) {
...
function enableSorting(gallery_id) {
	...
	$( ".gallery_items" ).sortable({
		stop: function(event, ui) {
			...
			$.ajax({
			  type: 'POST',
			  url: '/minerva_gallery/items/order/' + gallery_id + '.json',
...
function enableAgileUploader(gallery_id) {
	$('#agile_uploader').agileUploader({
		flashSrc: '/minerva_gallery/swf/agile-uploader.swf',
		flashWidth: 66,
		removeIcon: '/minerva_gallery/img/trash-icon.png',
		genericFileIcon: '/minerva_gallery/img/file-icon.png',
		submitRedirect: '/minerva_gallery/admin/items/manage/' + gallery_id,
		formId: 'upload-new-items',
		flashVars: {
			form_action: '/minerva_gallery/admin/items/create',
			file_limit: 20,
			file_filter: '*.jpg;*.jpeg;*.gif;*.png;*.JPG;*.JPEG;*.GIF;*.PNG',
			resize: 'jpg,jpeg,gif',
			force_preview_thumbnail: 'true',
			firebug: 'false',
			button_up:'/minerva_gallery/img/add-file-normal.png',
			button_over:'/minerva_gallery/img/add-file-over.png',
			button_down:'/minerva_gallery/img/add-file-over.png'	
		}
	});
}
...
$(document).ready(function() { 
	...
	$('#edit_item_form').submit(function() {
		$.ajax({
		  type: 'POST',
		  url: '/minerva_gallery/items/meta/' + $('#edit_item_id').val() + '.json',
